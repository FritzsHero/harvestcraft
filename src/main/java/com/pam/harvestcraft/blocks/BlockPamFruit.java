...
public class BlockPamFruit extends Block  implements IGrowable
{
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		if (((Integer) state.getValue(AGE)).intValue() == 2)
		{
            ...
            if(currentBlock == BlockRegistry.pamBreadfruit){
                savedStack = new ItemStack(ItemRegistry.breadfruitItem);
            } else
			...
			if(currentBlock == BlockRegistry.pamGuava){
                savedStack = new ItemStack(ItemRegistry.guavaItem);
            } else
			if(currentBlock == BlockRegistry.pamHazelnut){
                savedStack = new ItemStack(ItemRegistry.hazelnutItem);
			} else
			if(currentBlock == BlockRegistry.pamJackfruit){
                savedStack = new ItemStack(ItemRegistry.jackfruitItem);
            } else
			...
			if(currentBlock == BlockRegistry.pamLychee){
                savedStack = new ItemStack(ItemRegistry.lycheeItem);
            } else
			...
			if(currentBlock == BlockRegistry.pamRambutan){
                savedStack = new ItemStack(ItemRegistry.rambutanItem);
            } else
			if(currentBlock == BlockRegistry.pamSoursop){
                savedStack = new ItemStack(ItemRegistry.soursopItem);
            } else
            if(currentBlock == BlockRegistry.pamStarfruit){
                savedStack = new ItemStack(ItemRegistry.starfruitItem);
            } else
			if(currentBlock == BlockRegistry.pamTamarind){
				savedStack = new ItemStack(ItemRegistry.tamarindItem);
			} else
